...
public class HibernateCalificacionDAO extends HibernateGenericDAO<Calificacion> implements BICalificacionDAO {
 ...
 public boolean isCreated(CalificacionDTO calificacion) {
  return false;
  /*Query q = this.getEntityManager().createQuery("SELECT c FROM Calificacion c WHERE c.usuario = :usuario");
  q.setParameter("usuario", FactoryDAO.getFactoryDAO().getUsuarioDAO().find(calificacion.getUsuario().getId()));
  return (!q.getResultList().isEmpty());*/
 }
 
 @Override
 public void create(CalificacionDTO calificacionDTO) {
  Usuario usuario = (Usuario) FactoryDAO.getFactoryDAO().getUsuarioDAO().find(calificacionDTO.getUsuario().getId());
